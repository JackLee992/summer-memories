...
2026-09-21，所有者判断当前成果距离商业化仍远，要求整理进度并提交推送。此前获授权的资料研究、Blender动画与Seedance视频试验已经归档；主游戏开发继续暂停。详见 `docs/progress-checkpoint-2026-09-21.md` 与 `docs/motion/README.md`。在所有者另行要求前，不主动开发、生成素材、重试下载、轮询已完成任务或启动构建；不创建后台跟进。下文方向保留为历史上下文。

## 0. 2026-09-20 所有者确认的当前 Demo 方向

最新追加方向（覆盖下行试作）：**横版 2D 小关卡**，以切人协同、复制和回溯为核心，参考 ICEY 的横版表现范式；见 `docs/decisions/0005-side-scrolling-slice.md`。

此前试作方向：在已有内容上试作 **2.5D 场景＋2D 人物＋固定斜俯视＋可暂停部署**，具体见 `docs/decisions/0004-2-5d-tactical-trial.md`。Unity 技术边界继续适用；保留已提交的 3D 原型作为比较基线。
